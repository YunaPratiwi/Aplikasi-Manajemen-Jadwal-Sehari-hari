...
<form method="GET" action="{{ route('archive.tasks') }}" class="card mb-6 p-4">
...
<div id="task-list" class="space-y-4">
    @forelse($tasks as $task)
        <div class="task-item priority-{{ $task->priority_color }}" data-task-id="{{ $task->id }}">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-2">
                        <span class="priority-indicator priority-{{ $task->priority_color }}"></span>
                        <a href="{{ route('tasks.show', $task) }}" class="font-medium hover:underline truncate">{{ $task->title }}</a>
                        <span class="badge badge-warning">Diarsipkan</span>
                    </div>
                    <div class="text-sm text-gray-600 dark:text-gray-300 ml-8">
{{ $task->category?->name ?? '-' }} • Diarsipkan: {{ $task->archived_at->format('d M Y H:i') }}
                    </div>
                ...
                <div class="flex items-center gap-2 ml-4">
                    <form action="{{ route('tasks.unarchive', $task) }}" method="POST">
                        @csrf
                    ...
                    <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Hapus task ini secara permanen?')">
                        @csrf @method('DELETE')
