  ...
  <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-red-600 p-8 shadow-xl">
    ...
    <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between">
      ...
      <div class="mt-4 md:mt-0 flex items-center gap-3">
        <a href="{{ route('archive.categories') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white/10 backdrop-blur-sm text-white font-medium rounded-xl hover:bg-white/20 transition-all duration-200 border border-white/20">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path>
          </svg>
          Arsipkan
        </a>
        <a href="{{ route('categories.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-purple-600 font-medium rounded-xl hover:bg-gray-50 transition-all duration-200 shadow-md hover:shadow-lg">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $totalCategories ?? $categories->total() }}</p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $activeCategories ?? $categories->where('is_active', true)->count() }}</p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $inactiveCategories ?? $categories->where('is_active', false)->count() }}</p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $totalTasks ?? 0 }}</p>
      </div>
    </div>
  </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="flex flex-col justify-end space-y-3">
          <div class="flex items-center">
            <input type="checkbox" name="show_inactive" value="1" @checked(request('show_inactive')) id="show_inactive" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300 rounded">
      ...
      <div class="relative z-10">
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            <div class="relative">
              <div class="w-12 h-12 rounded-xl shadow-md" style="background-color: {{ $category->color }}"></div>
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                ...
                @endunless
                <span class="text-xs text-slate-500 dark:text-slate-400">{{ $category->tasks_count ?? 0 }} tugas</span>
              </div>
            </div>
          </div>
          <div class="flex items-center gap-1">
            <form action="{{ route('categories.toggle-active', $category) }}" method="POST">
              @csrf
              <button type="submit" class="p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-200" title="{{ $category->is_active ? 'Non-aktifkan' : 'Aktifkan' }}">
            ...
            <a href="{{ route('categories.edit', $category) }}" class="p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-200" title="Edit">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p class="text-slate-600 dark:text-slate-400 mb-4">{{ $category->description ?: 'Tidak ada deskripsi' }}</p>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700">
          <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
            <div class="flex items-center gap-1">
              ...
              <span>{{ $category->formatted_created_at ?? 'Tidak diketahui' }}</span>
            </div>
            @if($category->tasks_count > 0)
              <div class="flex items-center gap-1">
                ...
                <span>{{ $category->tasks_count }} tugas</span>
              </div>
            @endif
          </div>
          <div class="flex items-center gap-2">
            @if($category->completed_tasks_count > 0)
              <span class="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">
                {{ $category->completed_tasks_count }} selesai
              </span>
            @endif
            <a href="{{ route('categories.show', $category) }}" class="text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 text-sm font-medium transition-colors">
              Lihat Detail →
            </a>
          </div>
        </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-12 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10 text-center">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          @if(request('search'))
            <a href="{{ route('categories.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition-all duration-200">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
              </svg>
              Hapus Filter
          ...
          @endif
          <a href="{{ route('categories.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white font-medium rounded-xl hover:bg-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
            </svg>
            Buat Kategori Baru
  ...
  @if($categories->hasPages())
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        @else
          <a href="{{ $categories->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            Previous
        ...
        <div class="hidden md:flex items-center gap-1">
          @foreach($elements as $element)
            @if(is_string($element))
              <span class="px-4 py-2 text-sm text-slate-700 dark:text-slate-300">{{ $element }}</span>
            @else
              @if($element->isCurrent())
                <span class="relative z-10 inline-flex items-center px-4 py-2 text-sm font-medium bg-purple-600 text-white rounded-lg">{{ $element->page }}</span>
              @else
                <a href="{{ $element->url() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">{{ $element->page }}</a>
              @endif
            @endif
          @endforeach
        </div>
        ...
        @if($categories->hasMorePages())
          <a href="{{ $categories->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            Next
