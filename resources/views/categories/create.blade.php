        ...
        @if(isset($recentCategories) && $recentCategories->count() > 0)
          <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
            ...
            <div class="relative z-10">
              ...
              <div class="space-y-3">
                @foreach($recentCategories->take(3) as $recentCategory)
                  <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <div class="w-8 h-8 rounded-lg" style="background-color: {{ $recentCategory->color }}"></div>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-medium text-slate-900 dark:text-white truncate">{{ $recentCategory->name }}</p>
                      <p class="text-xs text-slate-500 dark:text-slate-400">{{ $recentCategory->tasks_count ?? 0 }} tugas</p>
                    </div>
                  </div>
                @endforeach
              </div>
