  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $categories->total() }}</p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $categories->count() > 0 ? round($categories->total() / max($categories->lastPage(), 1)) : 0 }}</p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10">
        ...
        <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $totalCategories ?? 0 }}</p>
      </div>
    </div>
  </div>
      ...
      <div class="relative z-10">
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-lg font-semibold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $category->name }}</h3>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700">
          <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
            ...
            <div class="flex items-center gap-1">
              ...
              <span>{{ $category->formatted_updated_at ?? 'Tidak diketahui' }}</span>
            </div>
          </div>
          <form action="{{ route('categories.toggleActive', $category) }}" method="POST">
            @csrf
  ...
  @empty
    <div class="relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-12 shadow-lg border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div class="relative z-10 text-center">
        ...
        <a href="{{ route('categories.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
          </svg>
          Kelola Kategori
          ...
          @foreach($elements as $element)
            ...
            @else
              @if($element->isCurrent())
                <span class="relative z-10 inline-flex items-center px-4 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg">{{ $element->page }}</span>
              @else
