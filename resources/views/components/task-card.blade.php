...
<div class="group relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-sm border border-slate-200/60 dark:border-slate-700/60 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-slate-300 dark:hover:border-slate-600">
    ...
    <div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b 
        @if($task->priority === 'high') from-red-500 to-red-600
        @elseif($task->priority === 'medium') from-amber-500 to-amber-600
    ...
    <div class="flex items-start justify-between mb-4">
        <div class="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div class="flex-1 min-w-0">
                <a href="{{ route('tasks.show', $task) }}" class="block group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                    <h3 class="font-semibold text-slate-900 dark:text-white truncate {{ $task->is_completed ? 'line-through opacity-60' : '' }}">
                        {{ $task->title }}
                    </h3>
                </a>
                @if($task->description)
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1 line-clamp-2 {{ $task->is_completed ? 'opacity-60' : '' }}">
        ...
        <div class="flex items-center gap-2 ml-4">
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                @if($task->priority === 'high') bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300
                @elseif($task->priority === 'medium') bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300
    ...
    <div class="flex items-center justify-between text-sm">
        <div class="flex items-center gap-4">
            @if($task->category)
                <div class="flex items-center gap-1.5 text-slate-600 dark:text-slate-400">
                    <div class="w-2 h-2 rounded-full" style="background-color: {{ $task->category?->color ?? '#6B7280' }}"></div>
            ...
            @if($task->due_date)
                <div class="flex items-center gap-1.5 
                    @if($task->due_date && \Carbon\Carbon::parse($task->due_date)->isPast() && !$task->is_completed) text-red-600 dark:text-red-400
                    @elseif($task->due_date && \Carbon\Carbon::parse($task->due_date)->isToday()) text-amber-600 dark:text-amber-400
        ...
        <!-- Status badge -->
        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
            @if($task->status === 'completed') bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300
            @elseif($task->status === 'in_progress') bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300
            @elseif($task->status === 'cancelled') bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300
    ...
    <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <div class="flex items-center gap-1">
            ...
            <form action="{{ route('tasks.destroy', $task) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus task ini?')">
                @csrf
                @method('DELETE')
                <button type="submit" 
